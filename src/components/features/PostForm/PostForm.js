	...
	<Form>
	  { formElements.map((el) => (
		<Form.Group key={ el.name }>
		  ...
		  <Form.Control type={ el.type } placeholder={ el.placeholder } as={ el.as }
						onChange={ (e) => onChangeHandler(e, el.name) } defaultValue={ data?.[el.name] }/>
		</Form.Group>)) }
		<Button onClick={() => formHandler(formState)} disabled={formState != null && Object.values(formState).every(x => !x)}>
			{buttonTitle}
		</Button>
